...
SELECT TENDT AS N'Tên đề tài', CAPQL N'Cấp quản lý',KINHPHI N'Kinh phí'
FROM DETAI

--CHO BIET DS CAC DE TAI CO KINH PHI 300 TRO LEN
SELECT *
FROM DETAI
WHERE KINHPHI > 300
--SAP XEP THEO TENDT TANG DAN VA KINH PHI GIAM DAN
ORDER BY TENDT ASC, KINHPHI DESC

--CHO DS CAC DE TAI MA CHUA CAP KINH PHI
SELECT *
FROM DETAI
WHERE KINHPHI IS NULL

--CHO DS GV HO TRAN
--SS TUONG DOI / SS THEO MAU
SELECT *
FROM GiAOVIEN
WHERE HOTEN LIKE N'Trần%'

--CHO DS GV VA TUOI
--NAM HIEN TAI - NAM SINH
SELECT MAGV,HOTEN,YEAR(GETDATE()) - YEAR(NGSINH) AS TUOI
FROM GiAOVIEN
WHERE YEAR(GETDATE()) - YEAR(NGSINH) >= 55
ORDER BY TUOI

SELECT MAGV,HOTEN,DATEDIFF(YYYY,NGSINH,GETDATE()) AS TUOI
FROM GiAOVIEN
WHERE DATEDIFF(YYYY,NGSINH,GETDATE()) >= 55
ORDER BY TUOI

SELECT MAGV,HOTEN
FROM GiAOVIEN 
WHERE MABM IN ('HTTT','HPT')

SELECT MAGV,HOTEN
FROM GiAOVIEN 
WHERE MABM = 'HTTT' OR MABM = 'HPT'

--CHO DS CAC GV CHUA CHU NHIEM DE TAI
--LAY DS TAT CA GV SUB LAY DS GV DA CNDT
SELECT MAGV, HOTEN FROM GiAOVIEN
EXCEPT
SELECT DISTINCT GVCNDT,HOTEN FROM DETAI, GiAOVIEN
WHERE DETAI.GVCNDT = GiAOVIEN.MAGV

SELECT DISTINCT GVCNDT,HOTEN FROM DETAI JOIN GiAOVIEN ON DETAI.GVCNDT = GiAOVIEN.MAGV

SELECT * FROM DETAI LEFT JOIN GiAOVIEN ON DETAI.GVCNDT = GiAOVIEN.MAGV

--CHO BIET TRUONG BO MON CO THAM GIA DE TAI
